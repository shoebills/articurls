...
export function SubscribeToAuthor({
  ...
  const formBody = (
    <>
      {status === "success" && message ? (
        ...
        <form onSubmit={onSubmit} className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end max-w-md mx-auto">
      ...
      {disclaimer ? (
        <p className="mt-3 text-xs text-muted-foreground text-center">{disclaimer}</p>
      ) : null}
    </>
  );
  ...
  return (
    <>
      <div className={className ?? "py-8 space-y-3 text-center"}>
        <p className="text-xl font-bold tracking-tight text-foreground text-center">{displayHeadline}</p>
        <p className="text-base text-muted-foreground text-center max-w-xl mx-auto leading-relaxed">{displayDescription}</p>
        {formBody}
      </div>
      {errorToast}
    </>
